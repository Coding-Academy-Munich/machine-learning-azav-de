...
# %%
max_contribution = max(contributions, key=abs)

# %%
max_index = contributions.index(max_contribution)
...
# %% [markdown]
#
# Berechne Preis für car1

# %%
price = 0
for feature, value, coef in zip(car_features, car1, car_coefficients):
    contribution = value * coef
    price += contribution
    print(f"{feature:20} : {value:5} × {coef:+6.2f} = {contribution:+7.2f}k€")

# %%
price += car_intercept
print(f"{'Base price':20} :                  = {car_intercept:+7.2f}k€")
print("-" * 50)
print(f"{'Total price':20} :                  = {price:7.2f}k€")
...
# %%
def predict_car_price(features, coefficients, intercept):
    """Calculate car price given features and model parameters."""
    price = sum(f * c for f, c in zip(features, coefficients))
    return price + intercept

# %%
for name, car in zip(car_names, cars):
    price = predict_car_price(car, car_coefficients, car_intercept)
    print(f"{name}: {price:.1f}k€")
    print(f"  Features: {car}")
    print()
...
# %%
print("Car B - Feature Contributions:")
print("-" * 50)
for feature, value, coef in zip(car_features, car_b, car_coefficients):
    contribution = value * coef
    contributions.append((feature, contribution))
    print(f"{feature:20} : {value:5} × {coef:+6.2f} = {contribution:+7.2f}k€")

# %%
print(f"{'Base price':20} :{' ' * 16}= {car_intercept:+7.2f}k€")

# %% [markdown]
#
# Finde größten Beitrag

# %%
contributions.sort(key=lambda x: abs(x[1]), reverse=True)
print(f"\nLargest contribution: {contributions[0][0]} ({contributions[0][1]:+.1f}k€)")
